...
def save_model(model, filename='linear_model.joblib'):
    """
...
def load_model(filename='linear_model.joblib'):
    """
...
if __name__ == "__main__":
    ...
    if data is not None:
        ...
        if X is not None and y is not None:
            ...
            # Step 5: Load the model and confirm it works
            loaded_model = load_model('linear_model.joblib')
            
            # Example prediction to confirm the model works after loading
            sample_data = X.iloc[:70] # Take a single row for testing
            prediction = loaded_model.predict(X)
            prediction_1= loaded_model.predict(sample_data)
            print("Prediction on sample data:", prediction_1)
            print("Predicion del modelo", prediction)
